...
EOF
    ...
    # 拉取 Docker 镜像并运行
    if docker pull registry.cn-hangzhou.aliyuncs.com/johnsonchasm/chasm-scout; then
        docker run -d --restart=always --env-file ./.env -p $PORT:$PORT --name scout registry.cn-hangzhou.aliyuncs.com/johnsonchasm/chasm-scout
    else
...
function restart_node() {
    ...
    # 拉取 Docker 镜像并重新运行
    if docker pull registry.cn-hangzhou.aliyuncs.com/johnsonchasm/chasm-scout; then
        docker run -d --restart=always --env-file ./.env -p 3001:3001 --name scout registry.cn-hangzhou.aliyuncs.com/johnsonchasm/chasm-scout
        echo "节点已成功重启。"
    else
        echo "拉取 Docker 镜像失败，请检查网络或稍后重试。"
        exit 1
